...
Shao et al. validated this at urban scale, demonstrating that GraphSAGE models incorporating interpretable physical edge features (inter-building distance, angular relations) significantly improved accuracy and robustness compared to ANN baselines \cite{ref51}. Explainability analysis showed that domain-relevant spatial features enhance interpretability for urban retrofit prioritization, enabling engineers to understand which building relationships drive energy consumption patterns.
...
\textbf{Zhang et al.: 4-Agent IDF Generation with 923 Sub-Agents}
Zhang et al. agentic workflow for automatic EnergyPlus IDF generation exemplifies the G-O-V architecture with unprecedented scale and specialization \cite{ref123}. The system uses four core agents:
\begin{enumerate}
...
\textbf{Lu et al.: Data2BEM 4-Agent Framework with HITL Loop}
Lu et al. Data2BEM framework demonstrates a complementary G-O-V architecture focused on calibration and retrofit analysis \cite{ref36}. The system uses four specialized agents:
\begin{enumerate}
...
\textbf{PhysicsGAN (Sisk et al.)}: Enforces physical constraints by penalizing generated data that violates surrogate-predicted constraints during training \cite{ref52}. For eVTOL takeoff trajectory design, the framework achieved:
\begin{itemize}
...
\end{itemize}
\textbf{Zhang et al.: Agentic EnergyPlus Workflow with Iterative Debugging}: Demonstrates domain-aligned reliability mechanisms for building simulation model generation through specialized agents and automatic error-correction loops \cite{ref123}. Performance metrics include:
\begin{itemize}
...
Fernandes et al. BIM-GPT assistant achieved 94\% success rate for single-function queries by integrating RAG with IFC parameter databases \cite{ref9}. The system retrieves relevant IFC entities and properties before generating responses, ensuring accuracy and reducing hallucinations \cite{ref9}. However, accuracy dropped to 49.5\% for complex queries requiring multi-step reasoning, highlighting that RAG alone is insufficient for complex tasks \cite{ref9}.
...
\textbf{Content}: Teach students to implement physics-constrained message passing in PI-GNNs, where messages propagated along edges represent heat flows governed by Fourier conduction ($Q_{cond} = kA\Delta T/(L\Delta t)$) and convection ($Q_{conv} = hA\Delta T$). Students embed energy balance constraints ($Q_{ext} + Q_1 + Q_2 + Q_3 + Q_4 = Mc\Delta T/\Delta t$) as hard architectural constraints, ensuring predictions satisfy thermodynamic laws by design rather than through loss function penalties.
\textbf{Learning Outcomes}:
\begin{itemize}
